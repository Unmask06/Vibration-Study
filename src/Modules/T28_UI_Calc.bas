...
Sub Calculate_All()
    ...
    For c = firstCol To lastCol
        ...
        Dim P1 As Double: P1 = ParameterManager.BargToPa(NzD(wsIn.Cells(idx("Upstream Static Pressure"), c).value))
        Dim Pshut As Double: Pshut = ParameterManager.BargToPa(NzD(wsIn.Cells(idx("Pump head at zero flow"), c).value))
        Dim Pv As Double: Pv = ParameterManager.BargToPa(NzD(wsIn.Cells(idx("Vapour Pressure"), c).value))
        Dim dP As Double: dP = ParameterManager.BargToPa(NzD(wsIn.Cells(idx("Static Pressure drop"), c).value))
